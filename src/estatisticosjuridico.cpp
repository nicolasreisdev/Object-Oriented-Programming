...
void EstatisticosJuridico::dados_mensal(){
    std::string cnpj, mes, mod, ano;
    int ano_int;
    bool mes_valido = false;
    bool cnpj_valido = false;
    bool ano_valida = false;
    ...
    if(mod == "1"){
        ...
        while (in.peek() != EOF){
            ...
            getline(in, aux2, ',');
            getline(in, aux3, '\n');
            aux3.erase(std::remove_if(aux3.begin(), aux3.end(), ::isspace), aux3.end());
    ...
    if(!ano_valida){
        std::cout << std::endl;
        std::cout << "Ano não registrado no Sistema!" << std::endl;
        return;
